...
export default function Button({isLight, cta, href}:{isLight:boolean, cta:string, href:string}) {
...
return(
  <motion.div className='w-max' initial="initial" whileHover="hovered" transition={{ duration: 0.4, ease: "easeInOut" }}>
    <Link className={isLight ? "flex flex-row font-medium bg-primary text-white py-2 px-4 justify-center rounded-md items-center gap-2" : "flex flex-row font-medium bg-background text-white py-2 px-4 border-1 justify-center rounded-md items-center gap-2" } href={href}>
      <span>{cta}</span>
